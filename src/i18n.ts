    ...
    terminalSnippet: `$ mainline context --current --json
{
  ...
  "suggested_next_step": "use the Postgres advisory-lock path"
    ...
    selfHub: {
      ...
      text: "The Mainline repo uses Mainline while building Mainline. This snapshot is generated from Git refs and notes on a schedule, then shipped as a static demo on mainline.sh.",
    ...
    terminalSnippet: `$ mainline context --current --json
{
  ...
  "suggested_next_step": "走 Postgres advisory-lock 路线"
    ...
    selfHub: {
      ...
      text: "Mainline 仓库在开发 Mainline 自己时也使用 Mainline。这份快照由 CI 定时从 Git refs 和 notes 生成，再作为静态 demo 发布到 mainline.sh。",
      cta: "打开实时 Hub",
      github: "查看仓库",
    ...
    terminalSnippet: `$ mainline context --current --json
{
  ...
  "suggested_next_step": "usar el camino con Postgres advisory locks"
    ...
    selfHub: {
      ...
      text: "El repo de Mainline usa Mainline mientras construye Mainline. Este snapshot se genera desde refs y notes de Git en CI y se publica como demo estático en mainline.sh.",
